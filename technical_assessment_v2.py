...
ut.AddLog("Remove duplicate based on SONG_ID and CLIENT_ID")
start_time = datetime.now()
df_10aug_dd = df_10aug.drop_duplicates(subset=['SONG_ID','CLIENT_ID'], keep='first')
end_time = datetime.now()
ut.AddLog("Need {} to remove the duplicate data".format(ut.CalcTime(start_time=start_time, end_time=end_time)))

ut.AddLog("Second step pivot, count distinct SONG_ID per CLIENT_ID")
start_time = datetime.now()
df_10aug_pv2 = pd.pivot_table(df_10aug_dd, values='SONG_ID', index='CLIENT_ID', aggfunc=np.count_nonzero)
